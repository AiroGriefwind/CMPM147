...
let eyeSFX1, eyeSFX2;
function p3_preload() {
  eyeSFX1 = loadSound('https://cdn.glitch.global/cbff2826-84fe-48f3-b3c7-26002bdcf1f0/gross-slimy-goo-foley-1-164289.mp3?v=1714771502030');
  eyeSFX2 = loadSound('https://cdn.glitch.global/cbff2826-84fe-48f3-b3c7-26002bdcf1f0/gross-slimy-goo-foley-7-164295.mp3?v=1714771500104');
  eyeSFX1.setVolume(0.3); 
  eyeSFX2.setVolume(0.3); 
}
...
function p3_tileClicked(i, j) {
    let key = `${i},${j}`;
    if (!clicks[key]) {
        clicks[key] = { count: 1, blinking: true, blinkTimer: 10 };  
    } else {
        clicks[key].blinking = true;  
        clicks[key].blinkTimer = 10;  
    }

    // Decide which sound effect to play based on the hash
    //Generated with the help of GPT
    if (XXH.h32("tile:" + key, worldSeed) % 4 == 0) {
        eyeSFX1.play();  // Play the first sound effect
    } else {
        eyeSFX2.play();  // Play the second sound effect
    }
}
...
function p3_drawTile(i, j) {
  ...
  endShape(CLOSE);
    ...
    let tileData = clicks[`${i},${j}`] || { blinking: false, blinkTimer: 0 };
  ...
  // Manage blinking
  //Generated with the help of GPT
    if (tileData.blinking) {
        let eyelidHeight = map(tileData.blinkTimer, 10, 0, 0, th / 2);
        fill(255);  // White part of the eye
        ellipse(0, 0, tw , th * 0.8 - eyelidHeight);  // Adjust height for blink

        fill(0);  // Pupil
        ellipse(tw / 4, 0, tw * 0.2, th * 0.2 - eyelidHeight / 2);  // Keep pupil aligned with blink

        // Update blink timer
        tileData.blinkTimer--;
        if (tileData.blinkTimer <= 0) {
            tileData.blinking = false;  // Stop blinking when timer ends
        }
    } else {
        fill(255);  // White part
    ellipse(0, 0, tw, th);
    fill(0);  // Pupil
    ellipse(tw / 4, 0, tw / 4, th / 4);
    }
